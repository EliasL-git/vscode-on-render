...
At startup, token values that include unsupported characters are converted to OpenVSCode-compatible values (SHA-256 hex when available, with a safe character substitution fallback) so startup does not fail.
